...
class ArmController:
    ...
    def __init__(self):
        ...
        for idx in range(getNumJoints(self.__platform_id)):
            resetJointState(
                self.__platform_id,
                idx,
                targetValue=ArmController.RESET_PLATFORM_JOINT_POSITIONS[idx],
                targetVelocity=0.0,
            )
            setJointMotorControl2(self.__platform_id, idx, VELOCITY_CONTROL)

        for idx in range(getNumJoints(self.__gripper_id)):
            resetJointState(
                self.__gripper_id,
                idx,
                targetValue=ArmController.RESET_GRIPPER_JOINT_POSITIONS[idx],
                targetVelocity=0.0,
            )
            setJointMotorControl2(self.__gripper_id, idx, VELOCITY_CONTROL)
        """
